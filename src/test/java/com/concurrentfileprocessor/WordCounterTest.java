...
class WordCounterTest {
    ...
    void testCountWordsInFile() throws FileNotFoundException {
        WordCounter.countFileComponents(tempFile, wordCount, totalCharacterCount);
    ...
    void testTotalCharacterCountThreadSafety() throws Exception {
        ...
        Thread t1 = new Thread(() -> {
            try { WordCounter.countFileComponents(tempFile1, wc, tc); } catch (FileNotFoundException ignored) {}
        });
        Thread t2 = new Thread(() -> {
            try { WordCounter.countFileComponents(tempFile2, wc, tc); } catch (FileNotFoundException ignored) {}
        });
        t1.start();
        t2.start();
        t1.join();
